...
class Game:
    ...
    def __init__(self, default_attribute='Sanity'):
        self.condition = 100
    ...
    def _act_and_transition(self, place: Place) -> None:
        ...
        # Use items
        if inventory:
            print(colored(Fore.GREEN, str(len(activities) + len(transitions) + 1)), "Check inventory")

        choice_number = _get_numeric('Choose one, or enter 0 to exit: ', len(activities) + len(transitions) + len(inventory))
        print("--------------------------------------------------------------")
        if choice_number == 0:
            exit(0)
            ...
            if isinstance(change, int):
                ...
                print(f"You have acquired {change}")
                for i in self.location.inventory_items:
                    if i.name == change:
                        self.inventory.append(i)
                        self.location.inventory_items.remove(i)
                        # print("Item added successfully")
        elif choice_number == (len(activities) + len(transitions) + 1):
            self._use_item()
        else:
    ...
    def _use_item(self):
        print('Choose and item to see:')
        
        inventory = self.inventory
        for index, inv in enumerate(inventory):
            print(colored(Fore.GREEN, str(index + 1)), inv.name)
        
        choice_number = _get_numeric('Choose one, or enter 0 to exit: ', len(inventory))
        
        if choice_number == 0:
            exit(0)
        else:
            item = inventory[choice_number - 1]
